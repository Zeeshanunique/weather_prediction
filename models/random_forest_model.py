...
class RandomForestModel:
    def __init__(self, n_estimators=100, max_depth=None, name="RandomForest"):
    ...
    def train(self, X_train, y_train):
        ...
        if len(X_train.shape) == 3:
            # For random forest, we'll use the last time step of each sequence
            X_train = X_train[:, -1, :]
    ...
    def feature_importance(self, feature_names=None):
        ...
        importance = np.zeros(self.model[0].n_features_in_)
        
        for model in self.model:
            importance += model.feature_importances_
        
        importance /= len(self.model)
    ...
    def plot_feature_importance(self, feature_names=None, title=None, save_path=None):
        ...
        plt.title(title or f'{self.name} Feature Importance')
